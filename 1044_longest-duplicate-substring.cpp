...
public:
    int valid(const string& S, int w, vector<unsigned long long>& magic, vector<unsigned long long>& RKHashCache) {
        ...
        for (int i = w - 1; i != S.size(); ++i) {
            RKHash = ((RKHash * 256) % 28379606267245447 + (S[i] - 'a')) % 28379606267245447;
    ...
    string longestDupSubstring(string S) {
        ...
        for (int i = 1; i != S.size(); ++i) {
            magic[i] = (magic[i - 1] * 256) % 28379606267245447;
            RKHashCache[i] = ((RKHashCache[i - 1] * 256) % 28379606267245447 + (S[i] - 'a')) % 28379606267245447;
        }
        int b = 0, e = S.size() - 1;
        while (b != e) {
